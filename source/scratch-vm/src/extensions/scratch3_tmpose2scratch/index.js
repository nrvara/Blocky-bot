...
const Message = {
  ...
  pose_classification_sample_model_url: {
    ...
    en: " ",
    ko: " ",
    "zh-tw": "https://teachablemachine.withgoogle.com/models/aqQcgCOtq/",
  },
  classify_pose: {
    ja: "ポーズを推定する",
    "ja-Hira": "ポーズをすいていする",
  ...
  set_confidence_threshold: {
    ...
    "zh-tw": "設定置信度閥值 [CONFIDENCE_THRESHOLD]",
  },
...
class Scratch3TMPose2ScratchBlocks {
        ...
        },
        {
          ...
          arguments: {
            URL: {
              type: ArgumentType.STRING,
              defaultValue:
                Message.pose_classification_sample_model_url[this.locale],
            },
          },
        },
        {
  ...
  poseLabelConfidence(args) {
    ...
    const entry = this.poseProbableLabels.find(
      (element) => element.className === args.LABEL
    );
  ...
  loadPoseClassificationModelFromURL(url) {
    return new Promise((resolve) => {
      fetch(`${url}metadata.json`)
        ...
        .then((metadata) => {
          if (
            url === this.poseModelUrl &&
            new Date(metadata.timeStamp).getTime() ===
              new Date(this.poseMetadata.timeStamp).getTime()
          ) {
  ...
  getPoseLabel() {
    if (!this.poseProbableLabels || this.poseProbableLabels.length === 0)
      return "";
    const mostOne = this.poseProbableLabels.reduce((prev, cur) =>
      prev.probability < cur.probability ? cur : prev
    );
    return mostOne.probability >= this.confidenceThreshold
      ? mostOne.className
      : "";
  }
  ...
  classifyPoseInVideo() {
    ...
    return this.classifyPose(
      this.runtime.ioDevices.video.getFrame({ mirror: true }),
      true
    );
  }
