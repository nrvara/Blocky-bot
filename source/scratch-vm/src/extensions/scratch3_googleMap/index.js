...
const googleMapsClient = new Client({
  key: "AIzaSyDQyvBdtQG4wBygmCMLjNhniJXd55PXLyA",
});
...
class googleMap {
  ...
  showMarker1(args) {
    if (this.recordMarker == 1) {
      ...
      var openGoogleMapWindow = window.open(
        "",
        "Google Map 擴充功能",
        "width=" +
          width +
          ", height=" +
          height +
          ", toolbar=no, scrollbars=no, menubar=no, name=no, status=no"
      );

      openGoogleMapWindow.document.write(`
        <head>
            <meta charset="utf-8" />
            <title>Google Map Extension</title>
            <style>
                #map {
                    height: 100%;
                }
      ...
      openGoogleMapWindow.document.write(`
                    ...
                    function setData(markerData) {
                        var sidebar_html = 'Coordinate List:<br>';
                        for (var i = 0; i < markerData.length; i++) {
  ...
  calculateDistance(args) {
    ...
    if (args.display == 1) {
      ...
      promise.then(function () {
        that.distanceKM = distanceKMValue;
      });
    }
    ...
    if (args.display == 0) {
      const origin =
        "'" +
        this.recordCoordinate[0][1] +
        "," +
        this.recordCoordinate[0][2] +
        "'";
      const destination =
        "'" +
        this.recordCoordinate[1][1] +
        "," +
        this.recordCoordinate[1][2] +
        "'";
  ...
  showMarker2(args) {
    ...
    var openGoogleMapWindow = window.open(
      ...
      "width=" +
        ...
        ", height=" +
        height +
        ", toolbar=no, scrollbars=no, menubar=no, location=no, status=no"
    );
  ...
  showMarker3(args) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i][this.name]) {
        ...
        var lat = data[i][this.latitude];
        var color =
          data[i][this.color] == undefined ? "EA4335" : data[i][this.color];
        var remark = data[i][this.note] == undefined ? "" : data[i][this.note];
        var width = this.width;
        var length = this.length;
      }
